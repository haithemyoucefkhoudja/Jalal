...
import { useCreateMeetingMutation } from "@/hooks/useCreateMeeting";
import { useCreateRequestMutation } from "@/hooks/useCreateRequest";
    ...
    "strictImageAttachment",
    {
      ...
      onClientUploadComplete: async (res) => {
        ...
        mutation.mutate(request, {
          onError:(error:any)=>{
          setError('root', { message: 'Something went wrong:'.concat(error.response.data.error) })
        }, onSuccess:()=>{
  ...
  const onSubmit = async (values: z.infer<typeof TimeFormSchema>) => {
    ...
    const meeting = {
      ...
      appointment: getDateTime(new Date(values.appointment))
        ...
        mutation.mutate(meeting,{onError:(error:any)=>{
          setError('root', { message: 'Something went wrong:'.concat(error.response.data.error) })
        }, onSuccess:()=>{
        setCompleted(true)
        }})
      ...
      {!Iscompleted ?
        <div className="flex flex-col items-center space-y-4">
          ...
          <form className="flex flex-col  space-y-3  w-full justify-center items-center" onSubmit={handleSubmit(onSubmit)}>
            ...
            />
            <Button disabled={mutation.isLoading} type="submit" className="mt-4 bg-green-400 hover:bg-green-400/90">
              {!mutation.isLoading ? <span>Schedule Meeting</span> : <div className="w-4 h-4 rounded-full border-2 border-b-transparent animate-spin border-[inherit]" />}
            </Button>
          </form>
